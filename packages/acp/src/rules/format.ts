/**
 * Findings formatter — turns a list of `Finding` into the
 * model-facing nudge text injected into the next assistant turn (CC
 * backend) or prepended to the tool result (server backend).
 *
 * Each rule's body content is inlined when present so the model reads
 * the full rationale alongside the violation. When the rule defines a
 * `message` template, regex captures and per-violation context get
 * interpolated (`${1}..${9}`, `${match}`, `${line}`).
 */
