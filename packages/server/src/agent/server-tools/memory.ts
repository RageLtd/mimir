...
export const storeTypedMemory = async (
  ...
  // body — the trigger matches a task description far better than procedure
  // steps, which is what makes ambient trigger-matching mechanical. Facts
  // (and any playbook lacking a trigger) embed their content as before.
  const embedSource =
    type === "playbook" && trigger
      ? `${name ?? ""}\n${trigger}`.trim()
      : content;
  const embedding = await embedOne(embedSource);
