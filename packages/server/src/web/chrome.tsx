...
*{box-sizing:border-box}html{font:100%/1.5 system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:var(--bg);color:var(--text)}body{margin:0;min-height:100vh}a{color:inherit;text-underline-offset:.2em}a:hover{text-decoration-thickness:.14em}:focus-visible{outline:.18rem solid var(--accent);outline-offset:.2rem}.skip{position:absolute;left:.75rem;top:-4rem;z-index:2;padding:.5rem .75rem;background:var(--text);color:var(--bg)}.skip:focus{top:.75rem}.site-head{border-bottom:1px solid var(--line);background:var(--surface)}.head-inner,.page-foot{max-width:var(--max);margin:auto;padding:1rem 1.25rem;display:flex;align-items:center;justify-content:space-between;gap:1rem}.brand{font-weight:800;letter-spacing:-.04em;text-decoration:none}.head-nav{display:flex;gap:1rem;font-size:.9rem}.frame{max-width:var(--max);margin:auto;padding:2rem 1.25rem}.kicker{margin:0 0 .5rem;color:var(--accent);font-weight:700;font-size:.8rem;letter-spacing:.08em;text-transform:uppercase}h1{max-width:18ch;margin:0;font-size:clamp(2.25rem,8vw,4.75rem);line-height:1;letter-spacing:-.055em}h2{margin:.1rem 0 .5rem;font-size:1rem}.lede{max-width:58ch;margin:1.25rem 0;color:var(--muted);font-size:1.1rem}.button{display:inline-block;padding:.65rem 1rem;border:0;border-radius:.45rem;background:var(--accent);color:var(--accent-text);font:inherit;font-weight:700;text-decoration:none;cursor:pointer}.form-error,.notice{max-width:36rem;padding:.65rem;border-left:.25rem solid var(--danger);background:var(--surface)}.notice{border-color:var(--accent)}.muted{color:var(--muted);font-weight:400}.page-foot{border-top:1px solid var(--line);color:var(--muted);font-size:.8rem}
`;
...
const STYLE_GROUPS = {
  ...
  lists: `.items{display:grid;gap:.65rem;padding:0;list-style:none}.item{padding:.75rem;border:1px solid var(--line);border-radius:.4rem}.item form{margin-top:.5rem}`,
  listHeads: `.item-head{display:flex;align-items:start;justify-content:space-between;gap:1rem}`,
  status: `.status{display:grid;grid-template-columns:max-content 1fr;gap:.25rem 1rem}.status dt{font-weight:700}.status dd{margin:0}`,
  secret: `.secret{overflow-wrap:anywhere;font-family:ui-monospace,SFMono-Regular,monospace}`,
  filters: `.audit-filters{display:grid;gap:.4rem;margin:1rem 0 1.5rem}.audit-filters label{font-weight:700}.audit-filters input,.audit-filters select{width:100%;padding:.6rem .7rem;border:1px solid var(--line);border-radius:.35rem;background:var(--surface);color:var(--text);font:inherit}.audit-filters button{justify-self:start}@media(min-width:48rem){.audit-filters{grid-template-columns:max-content minmax(8rem,1fr);align-items:center}.audit-filters button{grid-column:2}}`,
  activity: `.audit-events time{color:var(--muted);font-size:.85rem}`,
  members: `.member-actions{display:flex;flex-wrap:wrap;gap:.5rem}`,
  ceremony: `.ceremony{display:grid;gap:.75rem}.ceremony [role=status]{min-height:1.5rem}`,
  memory: `.memory-actions,.memory-pages{display:flex;gap:.5rem;margin:.75rem 0}.memory-controls{display:grid;gap:.4rem;margin-bottom:1rem}.memory-controls label{font-weight:700}.memory-controls input,.memory-controls select{width:100%;padding:.6rem .7rem;border:1px solid var(--line);border-radius:.35rem;background:var(--surface);color:var(--text);font:inherit}.memory-item h3{font-size:.85rem;color:var(--accent)}.memory-item>p{white-space:pre-wrap;color:var(--text)}.memory-item details{margin-top:.5rem}@media(min-width:48rem){.memory-controls{grid-template-columns:max-content 1fr}.memory-controls h2{grid-column:1/-1}}`,
};

type StyleGroup = keyof typeof STYLE_GROUPS;
...
const pageStyles = (styles: StyleGroup[]) =>
  `${BASE_STYLES}${styles.map((style) => STYLE_GROUPS[style]).join("")}`;

export const pageRenderer = jsxRenderer(
  ({ children, title, description, scripts, styles }) => (
    <html lang="en">
      <head>
        <meta charset="utf-8" />
