/**
 * `mimir-codex-bin hygiene [--live] [--model <id>]` — run the memory
 * hygiene sweep over the LOCAL replica (ported from cc-plugin).
 *
 ...
 * lives at ~/.mimir/hygiene-state.json — deliberately SHARED with the
 * other distributions: they all sweep the same replica, so the decay
 * clock must be one clock.
 */
...
import { extractionConfig } from "@mimir/plugin-core/shared-config";
import {
  createOrgReplica,
...
export const runHygieneCommand = async (args: readonly string[]) => {
  ...
  if ("error" in parsed) {
    console.error(
      `${parsed.error}\nUsage: mimir-codex-bin hygiene [--live] [--model <id>]`,
    );
    return 1;
  }
