/**
 * File-context injection on reads — PreToolUse hook (ported from
 * cc-plugin).
 *
 * Codex has no Read tool — the model reads files through Bash
 * (`sed -n '1,240p' file`, `cat file`, …). This hook conservatively
 * parses those single-file read commands (tool-map's readFilePath);
 * when one matches, it builds cartographer info (symbols, imports,
 * dependents) from the LOCAL cart index plus semantically-related
 * memories from the local replica, and injects them as
 * `additionalContext` so the model frames the read with that knowledge.
 *
 * Dedup is hash-keyed, not path-keyed: the state file maps
 * `filePath → contentHash`, and re-injection happens only when the
 * index's content_hash differs from the cached value — the cache
 * invalidates naturally after an apply_patch → reindex cycle.
 *
...
import { readFilePath } from "./tool-map";
...
/**
 * Memory count for the summary line. Prefers the index's memoryCount;
 * falls back to counting top-level bullets in the formatted string.
 */
...
export const runFileContextHook = async () => {
  ...
  const filePath = readFilePath(input.tool_name, input.tool_input);
