/**
 * `mimir-codex-bin embed-backfill` — vectorize replica rows without
 * embeddings (imported corpus + rows stored while the embedder was
 * absent). Safe to re-run any time; already-embedded rows are never
 * touched. Port of cc-plugin's command.
 */
