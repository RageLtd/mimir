...
- mimir (HTTP) connects to mimir-server's /mcp endpoint. Exposes server introspection ONLY (\`mcp__mimir__read_mimir_logs\`) — the org memory store moved fully client-side (MIM-88): the local replica is the brain, and it syncs across the org as ciphertext envelopes the server cannot read.
