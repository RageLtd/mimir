 ...
 * Relocated from mimir-server by MIM-89: this was always client-shaped
 * code (BYOK keys live in the client's env), it just happened to run
 * server-side.
 *
 ...
 *
 * Local providers (vLLM, Ollama, LM Studio) use the same registration
 * helpers but discover models dynamically from /models endpoints, gated
 * by their standard env vars (VLLM_BASE_URL / OLLAMA_BASE_URL /
 * LMSTUDIO_BASE_URL).
 *
...
// ---------------------------------------------------------------------------
// Engine options — injected at init, replacing the server config import
// ---------------------------------------------------------------------------

export type RegistryOptions = {
  /** When true, OpenRouter requests carry `provider: { zdr: true }` so only
   *  zero-data-retention endpoints serve them. */
...
export async function initProviderRegistry(options?: RegistryOptions) {
  if (options) registryOptions = options;
  ...
  if (vllmBaseUrl) {
    ...
    log.info("vllm initialized", {
      baseUrl: vllmBaseUrl,
      count: models.length,
    });
  }

  const ollamaBaseUrl = Bun.env.OLLAMA_BASE_URL;
  if (ollamaBaseUrl) {
    registerProvider(
      "ollama",
      "@ai-sdk/openai",
  ...
  const lmstudioBaseUrl = Bun.env.LMSTUDIO_BASE_URL;
  if (lmstudioBaseUrl) {
    registerProvider(
      "lmstudio",
...
async function fetchModels(url: string) {
  ...
  if (err) {
    log.warn("/models fetch failed", { err: err.message, url });
    return [];
  }
  if (!res.ok) {
    log.warn("/models returned error", { url, status: res.status });
    return [];
  }
  ...
  if (parseErr) {
    log.warn("/models parse failed", { err: parseErr.message, url });
    return [];
  }
