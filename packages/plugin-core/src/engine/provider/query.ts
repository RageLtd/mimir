 ...
 * Relocated from mimir-server by MIM-89.
 *
 ...
 *
 * Deliberately NOT ported from the server (their consumers stayed or died
 * server-side): resolveEmbeddingModel, getSmallModelConfig,
 * getEmbeddingModelMetadata, fetchModelId.
 */
 ...
 * "ANTHROPIC_API_KEY"). Clients use this to decide which STANDARD env
 ...
 * registry maps — those cache SDKs holding long-lived keys, and a
 * per-request key must never enter shared state.
 *
...
export function resolveModelWithOverride(
  ...
  const fullModelId = bareNameToFullId.get(bareModelId) ?? bareModelId;
  log.info("resolved model with per-request key", {
    providerId,
    model: fullModelId,
    byok: true,
  });
 ...
 */
// Return annotation is a deliberate return-types-rule exception: TS
// normalizes the union of these object-literal returns with phantom
// `?: undefined` members, which is NOT assignable to the SDK's
// index-signature type at call sites. The annotation hides nothing —
// every branch is still checked against SharedV3ProviderOptions.
