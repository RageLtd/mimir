 ...
 *
 * Shared by every distribution's `user-memory-mcp` subcommand (hoisted
 * from cc-plugin when codex-plugin became a second consumer). The editor's
 * MCP client speaks JSON-RPC 2.0 over stdio; we read newline-delimited
 * JSON from stdin, write responses to stdout, log only to stderr (any
