 ...
 *
 * Tool names are merged across backends: Claude Code (`Bash`,
 * `Edit`/`Write`/`MultiEdit`), ACP (`create_terminal`/`terminal`,
 * `fs_write_text_file`/`write_text_file`), and OpenCode (`bash`,
 * `edit`/`write`/`apply_patch`). Rules authored against one backend
 * match on the equivalent tool on another.
 */
const EVENT_TO_TOOLS: Readonly<Record<string, ReadonlySet<string>>> = {
  bash: new Set([
    "Bash", // Claude Code
    "create_terminal", // ACP server backend
    "terminal", // ACP server backend
    "bash", // OpenCode
  ]),
  file: new Set([
    "Edit", // Claude Code
